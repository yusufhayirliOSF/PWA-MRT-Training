    ...
    useEffect(() => {
        ...
        const fetchStore = async () => {
            const res = await fetch(
                `${getAppOrigin()}/mobify/proxy/ocapi/s/<Key Value: Site ID>/dw/shop/v20_2/stores?latitude=${GEO_LOCATION.lat}&longitude=${GEO_LOCATION.long}&client_id=<Key Value: Commerce API client ID>`
            )
            if (res.ok) {
...
App.getProps = async ({api, res}) => {
    ...
    let privacyPolicy
            const result = await fetch(
                `${getAppOrigin()}/mobify/proxy/ocapi/s/pwa-training/dw/shop/v20_2/content/privacy-policy?client_id=c1c2266d-77ec-43b4-9cdc-a8e52d636b7b`
            )
